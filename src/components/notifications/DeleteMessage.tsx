...
function DeleteMessage({ text }: Props) {
    ...
    const { isDeleteModalOpen, toDelteSetId, setIsDeleteModalOpen, setToDeleteSetId, deleteSet } = useAppContext();

    return (
            <>
                <div className={` { ${ isDeleteModalOpen ? "fixed" : "hidden"} } inset-0 absolute z-10 w-vw h-vh bg-transparent backdrop-blur`}></div>
                <div className={`fixed ${ isDeleteModalOpen ? "top-[30%]" : "-top-full" } z-11 -translate-x-1/2 left-1/2 transition-all overflow-hidden bg-white text-left rounded-lg max-w-[290px] shadow-xl`}>
